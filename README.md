# Hi, I’m Harry 👋  
Product-minded builder working on OCR → AI → knowledge pipelines.

Currently focused on turning messy handwritten notes into structured, usable outputs using modern web infrastructure and AI-assisted development workflows.

---

## 🚀 What I’m working on

- **DoubleTake** — an OCR → AI → Notion / Google Docs pipeline for handwritten notes  
  - Built for real-world handwriting (not clean scans)  
  - Async, step-based job tracking with retries and idempotency  
  - Designed around reliability and product constraints, not demos

---

## 🧠 How I work

- Think in **systems and workflows**, not isolated features  
- Use AI as a **multiplier** for prototyping, refactors, and architectural exploration  
- Optimise for “boring but reliable” over clever but fragile  
- Translate fuzzy user pain into concrete product decisions

---

## 🛠️ Product stack

### Languages
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![SQL](https://img.shields.io/badge/-SQL-4479A1?style=flat-square&logo=postgresql&logoColor=white)

### Frameworks & runtime
![Next.js](https://img.shields.io/badge/-Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white)
![React](https://img.shields.io/badge/-React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat-square&logo=node.js&logoColor=white)

### Data, infra & styling
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![Prisma](https://img.shields.io/badge/-Prisma-2D3748?style=flat-square&logo=prisma&logoColor=white)
![Supabase](https://img.shields.io/badge/-Supabase-3FCF8E?style=flat-square&logo=supabase&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/-Tailwind-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white)

### Tooling & integrations
![Git](https://img.shields.io/badge/-Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/-GitHub-181717?style=flat-square&logo=github&logoColor=white)
![Trigger.dev](https://img.shields.io/badge/-Trigger.dev-111827?style=flat-square)
![OpenAI API](https://img.shields.io/badge/-OpenAI%20API-412991?style=flat-square&logo=openai&logoColor=white)
![Notion API](https://img.shields.io/badge/-Notion%20API-000000?style=flat-square&logo=notion&logoColor=white)
![Google APIs](https://img.shields.io/badge/-Google%20Docs%20%26%20Drive%20API-4285F4?style=flat-square&logo=google&logoColor=white)

Used to orchestrate OCR → AI cleanup → document writing pipelines with async background jobs.

---

## 🤖 AI-assisted development workflow

![Cursor](https://img.shields.io/badge/-Cursor-000000?style=flat-square)
![Claude Code](https://img.shields.io/badge/-Claude%20Code-111827?style=flat-square)
![OpenAI Codex](https://img.shields.io/badge/-OpenAI%20Codex-412991?style=flat-square&logo=openai&logoColor=white)

Used for rapid prototyping, refactors, architectural exploration, and translating product intent into working code.

---

## 💼 Projects

Check out the pinned repositories below for work that reflects how I think and build.

---

## 📈 Contribution Graph

[![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=Itsmeddeyc&theme=react-dark&hide_border=true)](https://github.com/Itsmeddeyc)


---

## 📫 How to reach me

- LinkedIn: [@Harry Medd](https://www.linkedin.com/in/harry-medd/)

Open to product-leaning roles, early-stage collaboration, and interesting problems.
